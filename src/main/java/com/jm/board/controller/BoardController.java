...
public class BoardController {
    ...
    @GetMapping("/restFront")
    public String restFront(Model model){// 비동기로
        return "/views/restboard/front";
    }

    @PostMapping("/write")
    public String write(Board board){ //
        boardcrud.boardInsert(board);
    ...
    public String boardDelete(HttpServletRequest req) { //
        // 로그인 안되어 있으면 인터셉터에서 처리 진입 못하게 처리해야 함
    ...
    public String detail(@PathVariable("idx") int idx ,Model model) { // ?idx=6
        ...
        String nlString = System.getProperty("line.separator").toString(); // 타입리프에 출바꿈
